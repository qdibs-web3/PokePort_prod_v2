...
const Chat = ({ user, isLoggedIn }) => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      if (response.status === 200) {
        // Directly set the messages from the response
  ...
  const sendMessage = async (event) => {
    ...
    const messageData = {
      username: user.username,  // Only use username here
      content: newMessage.trim(),
    };
  
    console.log('Sending:', messageData);  // Log the data being sent
  
    try {
      const response = await api.post('/api/chat', messageData);
  
      if (response.status === 201) {
        setMessages((prevMessages) => [...prevMessages, response.data]); // Update the message list with the new message
